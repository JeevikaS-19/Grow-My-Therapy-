...
const AccordionItem: React.FC<{ question: string; answer: string }> = ({ question, answer }) => {
  ...
  return (
    <div className="border-b border-[#1A1E22]/10 py-10 first:border-t">
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="w-full flex justify-start items-center gap-8 group text-left"
      >
        <span className="text-[2.2rem] font-light text-[#1A1E22] transition-transform duration-500 w-10 flex justify-center opacity-40"
          style={{ transform: isOpen ? 'rotate(45deg)' : 'none' }}>
          +
        </span>
        <span className="text-[1.8rem] md:text-[2.6rem] serif-font text-[#1A1E22] font-medium tracking-tight group-hover:opacity-70 transition-opacity">
          {question}
        </span>
      </button>
      <div className={`overflow-hidden transition-all duration-700 ease-in-out ${isOpen ? 'max-h-[500px] opacity-100 mt-6 ml-[4.5rem]' : 'max-h-0 opacity-0'}`}>
        <p className="text-[17px] font-light text-[#1A1E22]/80 leading-relaxed max-w-xl">
          {answer}
        </p>
      </div>
    </div>
  ...
  return (
    <section className="py-24 px-8 max-w-[1440px] mx-auto flex flex-col md:flex-row gap-16 md:gap-24 bg-[#F2F5F7]">
      ...
      <div className="w-full md:w-[60%] flex flex-col justify-center">
        <h2 className="text-4xl md:text-6xl serif-font mb-10 text-[#1A1E22] font-light">FAQs</h2>
