...
const CredentialItem: React.FC<{ title: string; content: string }> = ({ title, content }) => {
  ...
  return (
    <div className="border-b border-[#2D3339]/10 py-8 first:border-t">
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="w-full flex justify-between items-center text-left group"
      >
        <span className="text-lg md:text-xl font-light tracking-wide text-[#2D3339] uppercase">{title}</span>
        <span className="text-2xl font-light text-[#2D3339] transition-transform duration-300" style={{ transform: isOpen ? 'rotate(45deg)' : 'none' }}>+</span>
      </button>
      <div className={`overflow-hidden transition-all duration-500 ease-in-out ${isOpen ? 'max-h-96 opacity-100 mt-4' : 'max-h-0 opacity-0'}`}>
        <p
          className="pb-4 text-sm md:text-base font-light text-[#2D3339]/70 leading-relaxed max-w-3xl"
...
const ProfessionalBackground: React.FC = () => {
  return (
    <section className="py-24 px-10 bg-[#E0E5E9] flex flex-col items-center text-[#2D3339]">
